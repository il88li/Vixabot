...
async def get_group_members(client, group_identifier, limit=100):
    try:
        ...
        return [user for user in participants if not user.bot and user.id != client.get_me().id]
    except Exception as e:
    ...
    try:
        while (session.is_transfer_active and 
            ...
            for member in available_members:
                ...
                # تأخير بين العمليات
                await asyncio.sleep(60 / session.transfer_settings['adds_per_minute'])
