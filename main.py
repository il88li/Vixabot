...
async def create_statistics_message(session):
    ...
    stats_text = f"""📊 **إحصائيات النقل الحية**
    ...
    try:
        while (session.is_transfer_active and 
            ...
            for member in available_members:
                ...
                # تأخير بين العمليات
                delay = max(5, 60 / session.transfer_settings['adds_per_minute'])  # حد أدنى 5 ثواني
...
اختر أحد الخيارات للبدء:"""
    ...
    async def set_speed(self, event, session):
        """إعداد السرعة"""
        session.current_operation = 'set_speed'
        session.waiting_for_input = True
        
        await event.edit(
            "⚡ **سرعة النقل**\n\nأدخل عدد الأعضاء لكل دقيقة (1-20):",
            buttons=[[Button.inline("❌ إلغاء", b"back_main")]]
        )
    
    async def process_speed_input(self, event, session, speed_text):
        """معالجة إدخال السرعة"""
        try:
            speed = int(speed_text)
            if speed < 1 or speed > 20:
                await event.reply("❌ الرقم يجب أن يكون بين 1 و 20")
                session.waiting_for_input = True
                return
            
            session.transfer_settings['adds_per_minute'] = speed
            await event.reply(f"✅ **تم تعيين السرعة:** {speed} عضو/دقيقة", buttons=create_setup_keyboard())
            session.current_operation = None
    ...
    async def start_transfer(self, event, session):
        ...
        if not all([session.transfer_settings['source_group'], 
                   session.transfer_settings['target_group'],
        ...
        # بدء عملية النقل
        session.transfer_task = asyncio.create_task(transfer_process(session))
    ...
    async def show_setup(self, event, session):
        ...
        setup_text = f"""⚙️ **إعدادات النقل**

🔐 **الحساب:** {status}
📁 **المصدر:** {session.transfer_settings['source_group'] or 'غير محدد'}
🎯 **الهدف:** {session.transfer_settings['target_group'] or 'غير محدد'}
📊 **العدد المطلوب:** {session.transfer_settings['required_count']}
⚡ **السرعة:** {session.transfer_settings['adds_per_minute']} عضو/دقيقة
...
if __name__ == "__main__":
    print("🚀 بدء تشغيل بوت نقل الأعضاء...")
